      ...
      @else {
        <div class="session-list">
          @for (session of sessions(); track session.id) {
            <article>
              <div class="app-mark" aria-hidden="true">{{ session.appKey.slice(0, 2).toUpperCase() }}</div>
              <div class="details"><div class="title-row"><h2>{{ session.appKey }}</h2>@if (session.isCurrent) { <span class="badge">Current</span> }</div><strong>{{ session.deviceLabel || 'Unknown device' }}</strong><span>Signed in {{ session.createdAt | date:'medium' }}</span><span>Expires {{ session.expiresAt | date:'medium' }}</span></div>
