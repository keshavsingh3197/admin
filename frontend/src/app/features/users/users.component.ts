  ...
  template: `
    <div class="users-wrap">
      ...
      @if (loading()) { <p>Loading…</p> } @else {
        <brand-data-table [columns]="columns" [rows]="visibleUsers()" [trackBy]="trackById" searchPlaceholder="Search users…">
          <ng-template let-u><tr [class.inactive]="!u.isActive"><td>{{ u.email }}</td><td>{{ u.displayName }}</td><td>@for (r of u.roles; track r) { <span class="badge">{{ r }}</span> }</td><td>{{ u.isActive ? 'Active' : 'Disabled' }}</td><td>{{ u.twoFactorEnabled ? 'Enabled' : 'Not enrolled' }}</td><td>{{ u.lastLoginAt ? (u.lastLoginAt | date:'short') : 'Never' }}</td><td><button class="linkish" type="button" (click)="edit(u)">Edit</button></td></tr></ng-template>
...
export class UsersComponent implements OnInit {
  ...
  readonly users = signal<UserListItem[]>([]); readonly allRoles = signal<Role[]>(['Admin', 'Editor', 'Viewer']); readonly customRoles = signal<CustomRoleView[]>([]); readonly groups = signal<GroupView[]>([]); readonly appFilter = signal<string | null>(null); readonly loading = signal(true); readonly busy = signal(false); readonly message = signal<string | null>(null); readonly ok = signal(false); readonly showCreate = signal(false); readonly editingUser = signal<UserListItem | null>(null);
  readonly visibleUsers = computed(() => { const app = this.appFilter(); if (!app) return this.users(); const keys = scopedRoleKeys(this.customRoles(), app); const groupIds = new Set(this.groups().filter(g => g.roleKeys.some(k => keys.has(k))).map(g => g.id)); return this.users().filter(u => u.roles.includes('Admin') || u.customRoleKeys.some(k => keys.has(k)) || u.groupIds.some(id => groupIds.has(id))); });
  ...
  readonly columns: BrandTableColumn<UserListItem>[] = [{ key:'email',label:'Email',value:u=>u.email,sortable:true },{ key:'displayName',label:'Name',value:u=>u.displayName,sortable:true },{ key:'roles',label:'Roles',value:u=>u.roles.join(', '),filterable:true },{ key:'status',label:'Status',value:u=>u.isActive?'Active':'Disabled',filterable:true },{ key:'twoFactor',label:'2FA',value:u=>u.twoFactorEnabled?'Enabled':'Not enrolled',filterable:true },{ key:'lastLogin',label:'Last login',value:u=>u.lastLoginAt??'',sortable:true },{ key:'actions',label:'' }];
  trackById = (u: UserListItem) => u.id; selfId(): string | undefined { return this.auth.user()?.id; }
  ngOnInit(): void { this.appFilter.set(this.route.snapshot.queryParamMap.get('app')); this.reload(); this.api.roles().subscribe({next:r=>this.allRoles.set(r)}); this.rbac.listRoles().subscribe({next:r=>this.customRoles.set(r)}); this.rbac.listGroups().subscribe({next:g=>this.groups.set(g)}); }
